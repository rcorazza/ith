<div class="post <?php if ( in_category( 'Sponsored' )) {  ?>sponsored<?php } ?>">
	...
	<ul class="author-share">
		...
		<a href="http://twitter.com/<?php the_author_meta( 'description' ) ?>" target="_blank"><span class="genericon genericon-twitter"></span></a>	
				<?php }  else { ?>
			<?php } ?>
			
		
		
		<br /><?php the_time('m/d/Y g:i a') ?> in <?php if ( in_category( 'Quicklink' )) {  ?>
